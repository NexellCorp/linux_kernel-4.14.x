...
/* nxp3220 specific registers */
#define NX_TMU_REG_INTEN			0xB0
#define NX_TMU_REG_INTSTAT			0xB4
#define NX_TMU_REG_INTCLEAR			0xB8
#define NX_TMU_REG_EMUL_CON			0x100
...
static void exynos4210_tmu_control(struct platform_device *pdev, bool on)
{
	...
	if (data->soc == SOC_ARCH_NXP3220)
		writel(interrupt_en, data->base + NX_TMU_REG_INTEN);
	else
					 ...
					 int temp)
{
	...
	else if (data->soc == SOC_ARCH_NXP3220)
		emul_con = NX_TMU_REG_EMUL_CON;
	else
...
static void exynos4210_tmu_clear_irqs(struct exynos_tmu_data *data)
{
	...
	if (data->soc == SOC_ARCH_EXYNOS5260) {
		...
		tmu_intstat = NX_TMU_REG_INTSTAT;
		tmu_intclear = NX_TMU_REG_INTCLEAR;
	} else {
...
static int exynos_of_get_soc_type(struct device_node *np)
{
	...
	else if (of_device_is_compatible(np, "nexell,nxp3220-tmu"))
		return SOC_ARCH_NXP3220;

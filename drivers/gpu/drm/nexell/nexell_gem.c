				    ...
				    unsigned int *handle)
{
	...
	/* for debugfs: /sys/kernel/debug/dri/0/gem_names */
	if (!obj->name) {
		ret = idr_alloc(&drm->object_name_idr, obj, 1, 0, GFP_KERNEL);
		if (ret >= 0) {
			obj->name = ret;
			ret = 0;
		}
					...
					size_t size, unsigned int max_order)
{
	...
	high_gfp = (GFP_HIGHUSER | __GFP_ZERO | __GFP_NOWARN | __GFP_NORETRY)
		   & ~__GFP_RECLAIM;
				...
				size_t size)
{
	...
	while (remaining > 0) {
		...
		DRM_DEBUG_GEM("[%3d] %pad/%p, size:%zu, remain:%zu\n",
			i, &dma_addr, page_address(page), length, remaining);
...
static void nx_drm_gem_sys_free(struct nx_gem_object *nx_obj)
{
	...
	for_each_sg(sgt->sgl, sg, sgt->nents, i) {
		...
		DRM_DEBUG_GEM("[%3d] 0x%p, 0x%lx/0x%p, size:%ld\n",
			i, page,
			       ...
			       struct vm_area_struct *vma)
{
	...
	unsigned long pgoff = vma->vm_pgoff * PAGE_SIZE;
	struct scatterlist *sg;
	int i;
	int ret;
...
static int nx_drm_gem_sys_contig_alloc(struct nx_gem_object *nx_obj,
				       size_t size)
{
	struct drm_device *drm;
	struct sg_table *sgt;
	...
	DRM_DEBUG_GEM("%pad/0x%p -> 0x%p, size:%zu\n",
		&nx_obj->dma_addr, page_address(page), nx_obj->cpu_addr, size);
...
static void nx_drm_gem_sys_contig_free(struct nx_gem_object *nx_obj)
{
	...
	DRM_DEBUG_GEM("%pad/0x%p, size:%zu\n",
		&nx_obj->dma_addr, page_address(page), nx_obj->size);
...
static int nx_drm_gem_dma_alloc(struct nx_gem_object *nx_obj,
				size_t size)
{
...
static void nx_drm_gem_buf_free(struct nx_gem_object *nx_obj)
{
	...
	DRM_DEBUG_GEM("%pad/0x%p, attach:%s\n",
...
static int nx_drm_gem_vm_fault(struct vm_fault *vmf)
{
	...
	if (nx_obj->pages) {
		mutex_lock(&nx_obj->lock);
		pgoff = ((unsigned long)vmf->address -
					vma->vm_start) >> PAGE_SHIFT;
		__gem_page_dirty(nx_obj->pages + pgoff);
...
static const struct vm_operations_struct nx_gem_vm_ops = {
				  ...
				  struct vm_area_struct *vma)
{
	...
	return drm->driver->gem_prime_mmap(obj, vma);
}
...
static void *nx_drm_gem_dmabuf_vmap(struct dma_buf *dma_buf)
{
	...
	return drm->driver->gem_prime_vmap(obj);
}

static void nx_drm_gem_dmabuf_vunmap(struct dma_buf *dma_buf, void *vaddr)
{
	struct drm_gem_object *obj = dma_buf->priv;
	struct drm_device *drm = obj->dev;
	...
	drm->driver->gem_prime_vunmap(obj, vaddr);
}
					...
					size_t size, uint32_t flags)
{
	...
	DRM_DEBUG_GEM("size:%zu, flags:0x%x[%s]\n",
...
/*
 * struct drm_driver elements, refer to drm_gem_cma_dumb_create
 */
			   ...
			   struct drm_mode_create_dumb *args)
{
	...
	/* create gem handle */
	ret = nx_drm_gem_handle_create(drm, obj, file_priv, handle);
	if (ret)
...
struct sg_table *nx_drm_gem_prime_get_sg_table(struct drm_gem_object *obj)
{
	...
	if (noncontig) {
		list_for_each_entry_safe(page, tmp_page, &pages, lru) {
			...
			DRM_DEBUG_GEM("[%d] sg 0x%lx/0x%p, size:%d\n",
		...
		DRM_DEBUG_GEM("sg %pad/0x%p size:%d\n",
				...
				struct sg_table *sgt)
{
	...
	DRM_DEBUG_GEM("dma_addr:%pad, size:%zu\n",
...
int nx_drm_gem_fops_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	DRM_DEBUG_GEM("enter 0x%lx~0x%lx 0x%lx, pgoff 0x%lx\n",
			    ...
			    struct drm_file *file_priv)
{
	...
	DRM_DEBUG_GEM("size:%lld, flags:0x%x\n", args->size, flags);
			  ...
			  struct drm_file *file_priv)
{
	...
	DRM_DEBUG_GEM("enter flags: 0x%x [%s]\n",
			 ...
			 struct drm_file *file_priv)
{
	...
	DRM_DEBUG_GEM("get dma %pad/0x%p\n",
			&nx_obj->dma_addr, nx_obj->cpu_addr);

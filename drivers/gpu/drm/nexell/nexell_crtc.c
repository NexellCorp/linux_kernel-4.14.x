// SPDX-License-Identifier: GPL-2.0+
/*
 * Nexell DRM driver
 * Copyright (c) 2018 JungHyun Kim <jhkim@nexell.co.kr>
 */

#include <drm/drmP.h>
#include <drm/drm_crtc_helper.h>
#include <drm/drm_atomic.h>
#include <drm/drm_atomic_helper.h>
#include <linux/of.h>
#include <linux/of_graph.h>
#include <linux/of_address.h>

#include "nexell_drv.h"
#include "nexell_gem.h"
#include "nexell_fb.h"

/*
 * for multiple framebuffers.
 */
static int  fb_align_rgb = 1;
MODULE_PARM_DESC(fb_align, "frame buffer's align (0~4096)");

module_param_named(fb_align, fb_align_rgb, int, 0600);

#define	is_changed(s) (\
		s->mode_changed || s->active_changed || \
		s->connectors_changed ? true : false)

static void nx_drm_atomic_crtc_enable(struct drm_crtc *crtc,
				      struct drm_crtc_state *old_state)
{
	struct nx_drm_crtc *nx_crtc = to_nx_crtc(crtc);

	DRM_DEBUG_KMS("crtc.%d\n", nx_crtc->pipe);

	if (nx_crtc->ops && nx_crtc->ops->enable)
		nx_crtc->ops->enable(crtc);

	drm_crtc_vblank_on(crtc);
}

static void nx_drm_atomic_crtc_disable(struct drm_crtc *crtc,
				       struct drm_crtc_state *old_state)
{
	struct nx_drm_crtc *nx_crtc = to_nx_crtc(crtc);

	DRM_DEBUG_KMS("crtc.%d\n", nx_crtc->pipe);

	drm_crtc_vblank_off(crtc);

	if (nx_crtc->ops && nx_crtc->ops->disable)
		nx_crtc->ops->disable(crtc);

	if (crtc->state->event && !crtc->state->active) {
		spin_lock_irq(&crtc->dev->event_lock);
		drm_crtc_send_vblank_event(crtc, crtc->state->event);
		spin_unlock_irq(&crtc->dev->event_lock);
		crtc->state->event = NULL;
	}
}

static int nx_drm_crtc_atomic_check(struct drm_crtc *crtc,
				    struct drm_crtc_state *state)
{
	struct nx_drm_crtc *nx_crtc = to_nx_crtc(crtc);
	struct nx_drm_crtc_ops *ops = nx_crtc->ops;

	DRM_DEBUG_KMS("crtc.%d enable:%d, active:%d, changed:%d\n",
		nx_crtc->pipe, crtc->state->enable,
		crtc->state->active, crtc->state->active_changed);

	if (!state->enable)
		return 0;

	if (ops && ops->atomic_check)
		return ops->atomic_check(crtc, state);

	return 0;
}

static void nx_drm_crtc_atomic_begin(struct drm_crtc *crtc,
				     struct drm_crtc_state *old_crtc_state)
{
	struct nx_drm_crtc *nx_crtc = to_nx_crtc(crtc);
	unsigned long flags;

	DRM_DEBUG_KMS("crtc.%d enable:%d, active:%d, %s, %s\n",
		nx_crtc->pipe, crtc->state->enable, crtc->state->active,
		is_changed(crtc->state) ? "changed" : "no change",
		crtc->state->event ? "event" : "no event");

	/* page flip event */
	if (crtc->state->event) {
		WARN_ON(drm_crtc_vblank_get(crtc) != 0);
		spin_lock_irqsave(&crtc->dev->event_lock, flags);
		nx_crtc->event = crtc->state->event;
		spin_unlock_irqrestore(&crtc->dev->event_lock, flags);
		crtc->state->event = NULL;
	}

	if (!crtc->state->enable || !crtc->state->active)
		return;

	if (!is_changed(crtc->state))
		return;

	to_nx_plane(crtc->primary)->align = fb_align_rgb;

	if (nx_crtc->ops && nx_crtc->ops->begin)
		nx_crtc->ops->begin(crtc);
}

static void nx_drm_crtc_atomic_flush(struct drm_crtc *crtc,
				     struct drm_crtc_state *old_crtc_state)
{
	struct nx_drm_crtc *nx_crtc = to_nx_crtc(crtc);
	struct drm_pending_vblank_event *event = crtc->state->event;

	DRM_DEBUG_KMS("crtc.%d enable:%d, active:%d, changed:%d, %s\n",
		nx_crtc->pipe, crtc->state->enable,
		crtc->state->active, crtc->state->active_changed,
		crtc->state->event ? "event" : "no event");

	if (event) {
		crtc->state->event = NULL;

		WARN_ON(drm_crtc_vblank_get(crtc) != 0);
		spin_lock_irq(&crtc->dev->event_lock);
		drm_crtc_arm_vblank_event(crtc, event);
		spin_unlock_irq(&crtc->dev->event_lock);
	}

	if (nx_crtc->ops && nx_crtc->ops->flush)
		nx_crtc->ops->flush(crtc);
}

static struct drm_crtc_helper_funcs nx_crtc_helper_funcs = {
	.atomic_enable = nx_drm_atomic_crtc_enable,
	.atomic_disable = nx_drm_atomic_crtc_disable,
	.atomic_check = nx_drm_crtc_atomic_check,
	.atomic_begin = nx_drm_crtc_atomic_begin,
	.atomic_flush = nx_drm_crtc_atomic_flush,
};

static void nx_drm_crtc_irq_free(struct drm_device *drm, struct drm_crtc *crtc)
{
	struct nx_drm_crtc *nx_crtc = to_nx_crtc(crtc);

	devm_free_irq(drm->dev, nx_crtc->irq, crtc);
}

static void nx_drm_crtc_destroy(struct drm_crtc *crtc)
{
	struct nx_drm_crtc *nx_crtc = to_nx_crtc(crtc);

	DRM_DEBUG_KMS("crtc.%d, %p\n", nx_crtc->pipe, nx_crtc);

	if (nx_crtc->ops && nx_crtc->ops->destroy)
		nx_crtc->ops->destroy(crtc);

	nx_drm_crtc_irq_free(crtc->dev, crtc);
	drm_crtc_cleanup(crtc);
	kfree(nx_crtc);
}

static int nx_drm_crtc_enable_vblank(struct drm_crtc *crtc)
{
	struct nx_drm_crtc *nx_crtc = to_nx_crtc(crtc);

	DRM_DEBUG_KMS("CRTC.%d\n", nx_crtc->pipe);

	if (nx_crtc->irq_ops && nx_crtc->irq_ops->irq_enable)
		return nx_crtc->irq_ops->irq_enable(crtc, nx_crtc->pipe);

	return 0;
}

static void nx_drm_crtc_disable_vblank(struct drm_crtc *crtc)
{
	struct nx_drm_crtc *nx_crtc = to_nx_crtc(crtc);

	DRM_DEBUG_KMS("CRTC.%d\n", nx_crtc->pipe);

	if (nx_crtc->irq_ops && nx_crtc->irq_ops->irq_disable)
		nx_crtc->irq_ops->irq_disable(crtc, nx_crtc->pipe);
}

static u32 nx_drm_crtc_get_vblank_counter(struct drm_crtc *crtc)
{
	struct nx_drm_crtc *nx_crtc = to_nx_crtc(crtc);

	if (nx_crtc->irq_ops && nx_crtc->irq_ops->get_counter)
		return nx_crtc->irq_ops->get_counter(crtc, nx_crtc->pipe);

	return 0;
}

static struct drm_crtc_funcs nx_crtc_funcs = {
	.set_config = drm_atomic_helper_set_config,
	.page_flip = drm_atomic_helper_page_flip,
	.destroy = nx_drm_crtc_destroy,
	.reset = drm_atomic_helper_crtc_reset,
	.atomic_duplicate_state = drm_atomic_helper_crtc_duplicate_state,
	.atomic_destroy_state = drm_atomic_helper_crtc_destroy_state,
	.enable_vblank = nx_drm_crtc_enable_vblank,
	.disable_vblank = nx_drm_crtc_disable_vblank,
	.get_vblank_counter = nx_drm_crtc_get_vblank_counter,

};

#ifdef DEBUG_FPS_TIME
#define	SHOW_PERIOD_SEC		3
#define	FPS_HZ	(60)
#define	DUMP_FPS_TIME(p) {	\
	static long ts[MAX_CRTCS] = { 0, }, vb_count;	\
	long new = ktime_to_ms(ktime_get());	\
	if (0 == (vb_count++ % (FPS_HZ * SHOW_PERIOD_SEC)))	\
		pr_info("[dp.%d] %ld ms\n", p, new - ts[p]);	\
	ts[p] = new;	\
	}
#else
#define	DUMP_FPS_TIME(p)
#endif

static irqreturn_t nx_drm_crtc_irq_handler(int irq, void *arg)
{
	struct drm_crtc *crtc = arg;
	struct nx_drm_crtc *nx_crtc = to_nx_crtc(crtc);
	struct drm_pending_vblank_event *event = NULL;

	drm_crtc_handle_vblank(crtc);
	spin_lock(&crtc->dev->event_lock);

	event = nx_crtc->event;
	if (event) {
		drm_crtc_send_vblank_event(crtc, event);
		drm_crtc_vblank_put(crtc);
		nx_crtc->event = NULL;
	}

	spin_unlock(&crtc->dev->event_lock);

	DUMP_FPS_TIME(nx_crtc->pipe);

	if (nx_crtc->irq_ops && nx_crtc->irq_ops->irq_done)
		nx_crtc->irq_ops->irq_done(crtc, nx_crtc->pipe);

	return IRQ_HANDLED;
}

static int nx_drm_crtc_irq_request(struct drm_device *drm,
				   struct drm_crtc *crtc)
{
	struct nx_drm_crtc *nx_crtc = to_nx_crtc(crtc);
	unsigned long flags = IRQF_SHARED;
	int err;

	err = devm_request_irq(drm->dev, nx_crtc->irq,
				nx_drm_crtc_irq_handler,
				flags, dev_name(drm->dev), crtc);
	if (err < 0) {
		DRM_ERROR("Failed to request IRQ#%u: %d\n", nx_crtc->irq, err);
		return -EINVAL;
	}

	DRM_INFO("Request crtc.%d irq %d\n", nx_crtc->pipe, nx_crtc->irq);
	return err;
}

static int of_get_port_nums(struct drm_device *drm, int *pipe, int pipe_size)
{
	struct device *dev = drm->dev;
	struct device_node *node = dev->of_node;
	struct device_node *port;
	int num = 0;

	node = of_get_child_by_name(node, "ports");
	if (!node)
		return 1;

	for_each_child_of_node(node, port) {
		u32 port_id = 0;

		if (of_node_cmp(port->name, "port") != 0)
			continue;
		if (of_property_read_u32(port, "reg", &port_id))
			continue;

		pipe[num] = port_id;
		num++;

		if (num > (pipe_size - 1))
			break;
	}

	of_node_put(port);
	of_node_put(node);

	return num;
}

int nx_drm_crtc_create(struct drm_device *drm, int max_crtcs)
{
	struct nx_drm_private *private;
	struct nx_drm_crtc *nx_crtcs[MAX_CRTCS];
	int pipes[MAX_CRTCS] = { 0, };
	int num_crtcs;
	int align = fb_align_rgb;
	int i, ret = -EINVAL;

	/* get ports with 'reg' property */
	num_crtcs = of_get_port_nums(drm, pipes, max_crtcs);

	if (align <= PAGE_SIZE && align > 0)
		fb_align_rgb = align;
	else
		fb_align_rgb = 1;

	DRM_INFO("num of crtcs %d, FB align %d\n", num_crtcs, fb_align_rgb);

	private = drm->dev_private;

	/* setup crtc's planes */
	for (i = 0; i < num_crtcs; i++) {
		struct nx_drm_crtc *nx_crtc =
				kzalloc(sizeof(struct nx_drm_crtc), GFP_KERNEL);
		if (!nx_crtc)
			goto err_crtc;

		nx_crtc->pipe = pipes[i];

		ret = nx_drm_crtc_init(drm, &nx_crtc->crtc, nx_crtc->pipe);
		if (ret < 0) {
			if (ret == -ENOENT) {
				kfree(nx_crtc);
				continue;
			}
			goto err_crtc;
		}

		ret = nx_drm_crtc_irq_request(drm, &nx_crtc->crtc);
		if (ret)
			goto err_crtc;

		ret = nx_drm_planes_create(drm,
				&nx_crtc->crtc, nx_crtc->pipe, &nx_crtc_funcs);
		if (ret < 0) {
			if (ret == -ENOENT) {
				kfree(nx_crtc);
				continue;
			}
		}
		drm_crtc_helper_add(&nx_crtc->crtc, &nx_crtc_helper_funcs);

		nx_crtcs[i] = nx_crtc;
		private->num_crtcs++;
		DRM_DEBUG_KMS("crtc[%d]: pipe.%d\n", i, nx_crtc->pipe);
	}

	return 0;

err_crtc:
	for (i = 0; i < private->num_crtcs; i++)
		kfree(nx_crtcs[i]);

	return ret;
}


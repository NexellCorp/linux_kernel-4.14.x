			 ...
			 int context_id)
{
	...
	ret = get_user(boost_flag, &msg->boost_flag);

	if (ret < 0) {
		tzlog_print(TZLOG_ERROR, "Unable to access memory\n");
		return ret;
	}
...
int tzio_exchange_message(struct file *filp, struct tzio_message *__user msg)
{
	...
	__payload = kmalloc(TZIO_PAYLOAD_MAX, GFP_KERNEL);
	if (__payload == NULL) {
		tzlog_print(TZLOG_WARNING,
				"No memory to allocate tzio payload\n");
		tzio_context_put(context);
		return -ENOMEM;
	}
...
static int tzcma_open(struct inode *inode, struct file *file)
{
	...
	for (i = 0; i < TZCMA_ALLOC_MAX_COUNT; i++) {
		...
		memset(&g_tzcmas_internal[i], 0, sizeof(struct tzcma_info_internal));
#ifdef CONFIG_ARTIK_USE_DMA

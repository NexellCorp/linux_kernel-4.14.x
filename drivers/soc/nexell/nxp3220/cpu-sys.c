...
static unsigned int hpm_cpu_ro_sel;
static unsigned int hpm_cpu_config = 3;
static unsigned int hpm_core_ro_sel;
static unsigned int hpm_core_config = 3;
...
static int read_cpu_hpm(void)
{
	...
	val = (hpm_cpu_ro_sel << 16) | (hpm_cpu_config << 4) | 0x05;
...
static int read_core_hpm(void)
{
	...
	regmap_update_bits(ecid_mod->core_reg, 0x404, HPM_CONFIG,
			hpm_core_config << 4);
	regmap_update_bits(ecid_mod->core_reg, 0x404, HPM_RO_SEL,
			hpm_core_ro_sel << 16);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	status = kstrtouint(buf, 0, &hpm_core_ro_sel);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	status = kstrtouint(buf, 0, &hpm_core_config);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	status = kstrtouint(buf, 0, &hpm_cpu_config);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	status = kstrtouint(buf, 0, &hpm_cpu_ro_sel);

...
static void m_can_do_rx_poll_isr(struct net_device *dev)
{
	...
	psr = m_can_read(priv, M_CAN_PSR);
	while ((psr & PSR_ACT_MASK) != PSR_ACT_IDLE)
		psr = m_can_read(priv, M_CAN_PSR);
...
{
	priv->rx_mem = devm_kcalloc(priv->device, RX_FIFO_ELEM_SIZE,

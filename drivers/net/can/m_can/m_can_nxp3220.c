 ...
 * Copyright (c) 2019 JungHyun Kim <jhkim@nexell.co.kr>
 */
...
#define PSR_ACT_MASK	(0x3 << PSR_ACT_SHIFT)
#define PSR_ACT_IDLE	(1 << PSR_ACT_SHIFT)
#define PSR_ACT_RX	(2 << PSR_ACT_SHIFT)
#define PSR_ACT_TX	(3 << PSR_ACT_SHIFT)
...
struct m_can_rx_fifo {
	u32 rxfx;
	u32 id, dlc;
	u8 db[CANFD_MAX_DLEN] __attribute__((aligned(8)));
};

#define RX_FIFO_ELEM_SIZE	64
...
struct m_can_priv {
	...
	struct m_can_rx_fifo *mem_base;
	int rgi; /* rx mem get index */
	int rpi; /* rx mem put index */
};
...
static void m_can_read_mem(struct net_device *dev)
{
	struct net_device_stats *stats = &dev->stats;
	struct m_can_priv *priv = netdev_priv(dev);
	struct m_can_rx_fifo *rxp;
	struct canfd_frame *cf;
	struct sk_buff *skb;
	u32 id, dlc;
	int i;

	/* calculate the mem put index for where to read data */
	rxp = priv->mem_base + priv->rpi;
	...
	if (!(dlc & RX_BUF_FDF) && (id & RX_BUF_RTR)) {
		...
		for (i = 0; i < cf->len; i += 4)
			*(u32 *)(cf->data + i) = *(u32 *)(rxp->db + i);
	}

	priv->rpi++;
	if (priv->rpi > (RX_FIFO_ELEM_SIZE - 1))
		priv->rpi = 0;
...
static int m_can_do_rx_poll(struct net_device *dev, int quota)
{
	...
	if (priv->rpi == priv->rgi) {
		netdev_dbg(dev, "no messages in fifo0\n");
		return 0;
	}

	while ((priv->rpi != priv->rgi) && (quota > 0)) {
		m_can_read_mem(dev);
...
static void m_can_read_fifo(struct net_device *dev, u32 rxfs)
{
	...
	/* calculate the mem get index for where to write data */
	fgi = (rxfs & RXFS_FGI_MASK) >> RXFS_FGI_SHIFT;

	rxp = priv->mem_base + priv->rgi;
	rxp->id = m_can_fifo_read(priv, fgi, M_CAN_FIFO_ID);
	rxp->dlc = m_can_fifo_read(priv, fgi, M_CAN_FIFO_DLC);

	if (rxp->dlc & RX_BUF_FDF)
		len = can_dlc2len((rxp->dlc >> 16) & 0x0F);
	else
		len = get_can_dlc((rxp->dlc >> 16) & 0x0F);
	...
	priv->rgi++;
	if (priv->rgi > (RX_FIFO_ELEM_SIZE - 1))
		priv->rgi = 0;
}

static void m_can_do_rx_poll_isr(struct net_device *dev)
{
	struct m_can_priv *priv = netdev_priv(dev);
	u32 irqstatus;
	u32 rxfs;
	int ffl, pkts = 0, idlewait = 1000;
	...
	while ((m_can_read(priv, M_CAN_PSR) & PSR_ACT_MASK) != PSR_ACT_IDLE) {
		udelay(1);
		if (idlewait-- == 0)
			break;
	}
	...
	if (idlewait < 0)
		netdev_warn(dev, "M_CAN Bus Error !!!\n");
...
static void m_can_chip_config(struct net_device *dev)
{
	...
	if (!(priv->can.ctrlmode & CAN_CTRLMODE_BERR_REPORTING))
		m_can_write(priv, M_CAN_IE, IR_ALL_INT & ~(IR_ERR_LEC_31X));
	else
...
static int m_can_init_mem(struct m_can_priv *priv)
{
	spin_lock_init(&priv->lock);

	priv->mem_base = devm_kcalloc(priv->device, RX_FIFO_ELEM_SIZE,
				sizeof(struct m_can_rx_fifo), GFP_KERNEL);
	if (!priv->mem_base)
		return -ENOMEM;
...
MODULE_DESCRIPTION("CAN bus driver for NXP3220 M_CAN controller based Bosch");

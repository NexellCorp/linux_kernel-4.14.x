...
static int nx_bus_qos_parse_dt(struct device *dev,
			       struct axim_busqos *qos)
{
	...
	qos->use_axqos = of_property_read_bool(np, "use-axqos");
	of_property_read_u32(np, "awqos-value", &qos->awqos);
	of_property_read_u32(np, "arqos-value", &qos->arqos);

	dev_info(dev,
		"QoS use-axqos:%s, awqos:0x%x, arqos:0x%x\n",
		qos->use_axqos ? "ON " : "OFF", qos->awqos, qos->arqos);

	return 0;
}

static void nx_bus_qos_setup(struct axim_busqos *qos)
{
	struct regmap *regmap = qos->regmap;
	u32 mask, val;

	mask = AXIM_AXQOS_MASK | AXIM_AWQOS_MASK | AXIM_ARQOS_MASK;
	val = ((qos->use_axqos ? 1 : 0) << AXIM_AXQOS_SHIFT) |
...
static int nx_bus_qos_probe(struct platform_device *pdev)
{
	...
	ret = nx_bus_qos_parse_dt(dev, qos);
	if (ret)
		return ret;
...
static const struct of_device_id nx_bus_qos_match[] = {
	{ .compatible = "nexell,nxp3220-axim-busqos" },
	{ },
};
...
MODULE_DESCRIPTION("AXIM QoS driver for the Nexell");

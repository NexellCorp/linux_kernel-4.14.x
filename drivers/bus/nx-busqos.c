...
struct nx_debug_qos {
	const char *name;
	u32 type;
	struct nx_nic_qos *qos;
};

static struct nx_debug_qos nx_debug[] = {
	{ "ar_fc_mode", AR_FC_MODE, },
	{ "aw_fc_mode", AW_FC_MODE, },
	{ "ar_fc_enable", AR_FC_ENABLE, },
	{ "aw_fc_enable", AW_FC_ENABLE, },
	{ "ar_fc_target_latency", AR_FC_TARGET_LATENCY, },
	{ "aw_fc_target_latency", AW_FC_TARGET_LATENCY, },
	{ "ar_max_qos", AR_MAX_QOS, },
	{ "ar_min_qos", AR_MIN_QOS, },
	{ "aw_max_qos", AW_MAX_QOS, },
	{ "aw_min_qos", AW_MIN_QOS, },
};

static u32 qos_readl(struct nx_nic_qos *qos, u32 offset)
{
...
static void qos_writel(struct nx_nic_qos *qos, u32 offset, u32 value)
{
#ifndef QOS_SECURE_ACCESS
...
static int nx_nic_qos_parse_dt(struct platform_device *pdev,
			       struct nx_nic_qos *qos)
{
	...
	if (!of_property_read_u32(np, "ar_fc_mode", &val))
		qos->qos_control |=
			((val & QOS_CNTL_MASK_BITS) << QOS_CNTL_AR_FC);

	if (!of_property_read_u32(np, "aw_fc_mode", &val))
		qos->qos_control |=
			((val & QOS_CNTL_MASK_BITS) << QOS_CNTL_AW_FC);

	if (!of_property_read_u32(np, "ar_fc_enable", &val))
		qos->qos_control |=
			((val & QOS_CNTL_MASK_BITS) << QOS_CNTL_EN_AR_FC);

	if (!of_property_read_u32(np, "aw_fc_enable", &val))
		qos->qos_control |=
			((val & QOS_CNTL_MASK_BITS) << QOS_CNTL_EN_AW_FC);

	if (!of_property_read_u32(np, "ar_fc_target_latency", &val))
		qos->target_fc |=
			((val & QOS_TARGET_FC_MASK_BITS) << QOS_TARGET_FC_AR);

	if (!of_property_read_u32(np, "aw_fc_target_latency", &val))
		qos->target_fc |=
			((val & QOS_TARGET_FC_MASK_BITS) << QOS_TARGET_FC_AW);

	if (!of_property_read_u32(np, "ar_max_qos", &val))
		qos->qos_range |=
			((val & QOS_RANGE_BITS) << QOS_RANGE_AR_MAX);

	if (!of_property_read_u32(np, "ar_min_qos", &val))
		qos->qos_range |=
			((val & QOS_RANGE_BITS) << QOS_RANGE_AR_MIN);

	if (!of_property_read_u32(np, "aw_max_qos", &val))
		qos->qos_range |=
			((val & QOS_RANGE_BITS) << QOS_RANGE_AW_MAX);

	if (!of_property_read_u32(np, "aw_min_qos", &val))
		qos->qos_range |=
			((val & QOS_RANGE_BITS) << QOS_RANGE_AW_MIN);

	dev_dbg(dev, "[%s] QoS control:0x%x, target-fc:0x%x, range:0x%x\n",
...
static void nx_nic_qos_setup(struct nx_nic_qos *qos)
{
...
static int nx_nic_qos_get(void *data, u64 *val)
{
	struct nx_debug_qos *d_qos = data;
	struct nx_nic_qos *qos = d_qos->qos;
	u32 mask, offs;
	int shift, size;
	u32 v;
...
static int nx_nic_qos_set(void *data, u64 val)
{
	struct nx_debug_qos *d_qos = data;
	struct nx_nic_qos *qos = d_qos->qos;
	u32 mask, offs;
...
DEFINE_SIMPLE_ATTRIBUTE(nx_debug_qos_fops,
		nx_nic_qos_get, nx_nic_qos_set, "%llu\n");

static int nx_qos_debugfs_init(struct nx_nic_qos *qos)
{
	struct dentry *d_dir, *d_sub;
	struct device_node *np = qos->dev->of_node;
	umode_t mode = S_IRUGO | S_IWUSR;
	const char *dir_name = "nx_qos";
	int i;
...
static int nx_nic_qos_probe(struct platform_device *pdev)
{
	struct nx_nic_qos *qos;
	struct device *dev = &pdev->dev;
	int ret;
	...
	ret = nx_nic_qos_parse_dt(pdev, qos);
	if (ret)
		return ret;

	nx_nic_qos_setup(qos);
...
static int nx_nic_qos_remove(struct platform_device *pdev)
{
	return 0;
}

#ifdef CONFIG_PM_SLEEP
static int nx_nic_qos_suspend(struct device *dev)
{
	return 0;
}

static int nx_nic_qos_resume(struct device *dev)
{
	struct nx_nic_qos *qos = dev_get_drvdata(dev);
...
static const struct dev_pm_ops nx_nic_qos_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(nx_nic_qos_suspend, nx_nic_qos_resume)
};
#endif

static const struct of_device_id nx_nic_qos_match[] = {
	{ .compatible = "nexell,nxp3220-nic-qos" },
	{ },
};

static struct platform_driver nx_bus_qos_driver = {
	.probe		= nx_nic_qos_probe,
	.remove		= nx_nic_qos_remove,
	.driver		= {
		.name	= "nx-nic-qos",
		.owner	= THIS_MODULE,
#ifdef CONFIG_PM_SLEEP
		.pm	= &nx_nic_qos_pm_ops,
#endif
		.of_match_table = nx_nic_qos_match,
	},
};

static int __init nx_nic_qos_init(void)
{
	return platform_driver_register(&nx_bus_qos_driver);
}
arch_initcall(nx_nic_qos_init);

				      ...
				      struct device *dev, struct nx_clipper *me)
{
	...
	if (of_property_read_string(np, "sensor_name", &name)) {
		dev_err(dev, "failed to get sensor i2c name\n");
		return -EINVAL;
	}

	i2c_bus = of_parse_phandle(np, "i2c-bus", 0);
	if (!i2c_bus) {
		dev_err(dev, "No i2c-bus found\n");
		return -ENODEV;
	}

	i2c_adapter = of_find_i2c_adapter_by_node(i2c_bus);
	if (!i2c_adapter) {
...
#ifndef CONFIG_VIDEO_NXP3220_CLIPPER
		}
#endif
	}
				    ...
				    struct v4l2_subdev_selection *sel)
{
	struct nx_clipper *me = v4l2_get_subdevdata(sd);
	switch (sel->target) {
	case V4L2_SEL_TGT_COMPOSE: /* V4L2_TYPE_IS_OUTPUT */
	case V4L2_SEL_TGT_CROP:
		memcpy(&sel->r, &me->crop, sizeof(struct v4l2_rect));
		break;
	case V4L2_SEL_TGT_COMPOSE_BOUNDS:  /* V4L2_TYPE_IS_OUTPUT */
...
static int nx_clipper_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	int ret = 0;
...
static const struct v4l2_ctrl_ops nx_clipper_ctrl_ops = {
	.g_volatile_ctrl = nx_clipper_g_volatile_ctrl,
};
...
#define NUM_CTRLS 1
static int nx_clipper_init_ctrls(struct nx_clipper *me)
{
	struct v4l2_ctrl *ctrl_min_buffers;

	v4l2_ctrl_handler_init(&me->handler, NUM_CTRLS);

	ctrl_min_buffers = v4l2_ctrl_new_std(&me->handler,
				&nx_clipper_ctrl_ops,
			V4L2_CID_MIN_BUFFERS_FOR_CAPTURE, 1, 32, 1, 8);
	if (!ctrl_min_buffers) {
		dev_err(&me->pdev->dev, "%s: failed to get min buffers\n",
				__func__);
		return -ENOENT;
	}
	ctrl_min_buffers->flags |= V4L2_CTRL_FLAG_VOLATILE;

	me->subdev.ctrl_handler = &me->handler;
	if (me->handler.error) {
		dev_err(&me->pdev->dev, "%s: ctrl handler error(%d)\n",
...
static int init_v4l2_subdev(struct nx_clipper *me)
{
	...
	ret = nx_clipper_init_ctrls(me);
	if (ret < 0) {
		dev_err(&me->pdev->dev, "%s: failed to initialize controls\n",
			__func__);
		return ret;
	}
...
{
	int ret;
	struct i2c_adapter *adapter = me->sensor_info.i2c_adapter;
	...
	if (ret) {
		dev_err(&me->pdev->dev, "failed to setup link %s->%s,ret %d\n",
				sensor->entity.name, input->name, ret);
		goto error;
	}
...
static int register_v4l2(struct nx_clipper *me)
{
	...
	if (ret) {
		dev_err(&me->pdev->dev, "can't register v4l2 subdev!\n");
		return ret;
	}
	...
	if (!video) {
		dev_err(&me->pdev->dev, "can't create v4l2 video devce!\n");
		return -ENODEV;
	}
	...
	if (ret < 0) {
		dev_err(&me->pdev->dev, "can't create link %s->%s,ret %d\n",
				entity->name, video->vdev.entity.name, ret);
		return ret;
	}
	...
	if (ret) {
		dev_err(&me->pdev->dev, "can't setup link %s->%s,ret %d\n",
				entity->name, video->vdev.entity.name, ret);
		return ret;

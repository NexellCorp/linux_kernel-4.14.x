...
/* NTSC -> 720x480i */
static struct reg_val _sensor_init_data_720x480i[] = {
	{0x40, 0x00},
	{0x35, 0x25},
	{0x4E, 0x0D},
	{0xF5, 0x10},
	...
	{0x39, 0x04},
	{0x3A, 0x32},
	{0x02, 0xCF},// 8bit mode CF //16 BIT : 0x4F
	{0x6F, 0x02},
	{0x70, 0x00},
	{0x71, 0x00},
	{0xC2, 0x0F},
...
)
{
	...
	case TP2825_NTSC_HALF:
		...
		break;
#if 0
	case TP2825_NTSC_720:
		vmsg("## [%s()] TP2825_NTSC_720\n", __func__);
		tp2825_set_work_mode_NTSC(client);
		tp28xx_byte_write(client, 0x02, 0xCF);
		tmp = tp28xx_byte_read(client, 0x4E);
		tmp |= 0x04;
...
static int tp2825_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		if (me->first) {
			...
			} else if (mode == TP2825_NTSC_720) {
				vmsg("## [%s()] TP2825_NTSC_720\n", __func__);
				reg_val = _sensor_init_data_720x480i;
				while (reg_val->reg != 0xff) {
					tp28xx_byte_write(me->i2c_client,
						reg_val->reg, reg_val->val);
					reg_val++;
		...
		struct v4l2_subdev_format *fmt)
{
	...
	/* NTSC -> 960x480i */
		...
		mode = TP2825_NTSC_HALF;
	/* NTSC -> 720x480i */
	else if (me->width == 720 && me->height == 480)
		mode = TP2825_NTSC_720;
	/* PAL -> 1920x576i */
